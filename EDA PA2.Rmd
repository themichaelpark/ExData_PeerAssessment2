...
unzip("fnei.zip")
NEI <- readRDS("summarySCC_PM25.rds")
d <- subset(NEI, type == "ON-ROAD" & fips == "24510")
qplot(x = year, y = Emissions, data = d, stat = "summary", fun.y = "sum", geom = "line")
```
...
unzip("fnei.zip")
NEI <- readRDS("summarySCC_PM25.rds")
ggplot(subset(NEI,type == "ON-ROAD" & fips %in% c("24510", "06037")), aes(year, Emissions, group=fips, fill=fips)) + geom_line(stat = "summary", fun.y = "sum")
```
